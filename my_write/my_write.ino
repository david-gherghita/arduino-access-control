...
void loop(void) {
  ...
  uint8_t myAccessBits[3] = {0x7F, 0x07, 0x88};
  uint8_t idx = 0;
  ...
  if (success) 
  {
    ...
    for (idx = 0; idx < numOfSector; idx++)
    {
      ...
      success = nfc.mifareclassic_AuthenticateBlock (uid, uidLength, 4 * idx, 0, MY_KEY);
      if (!success)
      {
      ...
      memcpy(blockBuffer, DEFAULT_KEY, 6);
      memcpy(blockBuffer + 6, blankAccessBits, 3);
      blockBuffer[9] = 0x00;
      memcpy(blockBuffer + 10, DEFAULT_KEY, 6);
